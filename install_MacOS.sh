...
if command -v anaconda >/dev/null 2>&1; then
	...
	cd $tiitba/bin
	chmod +x tiitbaGUI.py AdditionalFunctions.py && cd 
	echo "--------------------------------------------------------------------------------------"
##  Desktop application
	...
	read ans
	if [ $ans == y ] || [ $ans == Y ] ; then
		file=$(curl -s https://repo.anaconda.com/archive/ | grep Anaconda3- | grep MacOSX-x86_64 | cut -d\" -f2)
		url='https://repo.anaconda.com/archive/'$file
		wget $url
		bash Anaconda3*-grep MacOSX-x86_64*
		rm Anaconda*
		########################################
	# installation path
		cond=0
		...
		# Python dependences for Anaconda Users 
		echo "--------------------------------------------------------------------------------------"
		echo 'Creating python environment and Installing Dependences"'
		conda env create -f tiitba_env.yml
